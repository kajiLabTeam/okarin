...
export const ibeaconConfigSchema = z.object({
  uuid: uuidSchema,
...
export const createBeaconRequestSchema = z.object({
...
}).strict().openapi('CreateBeaconRequest')

export const updateBeaconRequestSchema = z.object({
  ...
  if (Object.keys(value).length === 0) context.addIssue({ code: z.ZodIssueCode.custom, message: 'patch must not be empty' })
}).openapi('UpdateBeaconRequest')

export const beaconListResponseSchema = z.object({
  beacons: z.array(beaconSchema),
  pagination: z.object({ next_cursor: z.string().nullable(), total_count: z.number().int().nonnegative() }),
}).openapi('BeaconListResponse')

export const recordingBeaconConfigResponseSchema = z.object({
