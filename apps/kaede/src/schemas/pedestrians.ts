...
export const pedestrianSchema = z
  .object({
...
export const pedestriansListResponseSchema = z
  .object({
    pedestrians: z.array(pedestrianSchema).openapi({
      description: '計測対象として選択可能な pedestrian 一覧',
    }),
  })
  .openapi('PedestriansListResponse')

export const createPedestrianWithoutOrganizationRequestSchema = z
  .object({
    display_name: z.string().min(1).openapi({
      description: 'pedestrian を画面上で識別する表示名',
    }),
  ...
  .openapi('CreatePedestrianWithoutOrganizationRequest')

export const createPedestrianRequestSchema = createPedestrianWithoutOrganizationRequestSchema
  .extend({
    organization_id: uuidSchema.openapi({
      description: 'pedestrian を所属させる organization の ID',
    }),
  })
  .openapi('CreatePedestrianRequest')
