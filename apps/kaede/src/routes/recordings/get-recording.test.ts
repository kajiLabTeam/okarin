...
const { getRecordingMock } = vi.hoisted(() => ({
  getRecordingMock: vi.fn(),
}))

vi.mock('../../usecases/recordings/get-recording.js', () => ({
...
describe('GET /api/recordings/:recordingId', () => {
  ...
  it('recording 詳細を返す', async () => {
    ...
    const app = createRouteTestApp('/recordings', registerGetRecordingRoute, {
      actor: managerActor,
    })
    const response = await app.request(`/api/recordings/${recordingId}`)
    ...
    expect(getRecordingMock).toHaveBeenCalledWith(managerActor, {
      recordingId,
    })
  })
