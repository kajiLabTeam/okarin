...
import { organizationTrajectoriesResponseSchema } from '../../schemas/trajectories.js'
import { listOrganizationTrajectoriesForSession } from '../../usecases/organizations/index.js'
import { getSessionTokenFromCookie } from '../auth/cookie.js'
import { toOrganizationErrorResponse } from './error.js'

export const registerListOrganizationTrajectoriesRoute = (app: OpenAPIHono) => {
  const route = createRoute({
    method: 'get',
    path: '/{organizationId}/trajectories',
    tags: ['Organizations'],
    description: 'organization 内 trajectory 一覧を取得する',
    request: {
    ...
    responses: {
      200: {
        description: 'organization trajectories',
        content: {
          'application/json': {
            schema: organizationTrajectoriesResponseSchema,
          },
        },
      },
      400: {
