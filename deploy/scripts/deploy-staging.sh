...
DEPLOY_META_FILE="$RUNTIME_DIR/staging.env"
...
SEAWEEDFS_CONFIG_FILE="$REPO_DIR/deploy/seaweedfs/s3.staging.conf"
...
log "Starting deploy for env: staging"
...
log "Starting application services for env: staging"
compose_cmd up -d --build --remove-orphans

log "Waiting for services to become healthy for env: staging"
...
log "Deploy completed for staging at $(git rev-parse --short HEAD)"
